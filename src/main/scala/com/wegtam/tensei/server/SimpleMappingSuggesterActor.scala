...
object SimpleMappingSuggesterActor {
  def props(): Props = Props(classOf[SimpleMappingSuggesterActor])
}
...
class SimpleMappingSuggesterActor extends SimpleSuggester with Actor with ActorLogging {
  override def receive: Receive = {
    case MappingSuggesterMessages.SuggestMapping(cookbook, mode, answerTo) =>
      ...
      if (mode != MappingSuggesterModes.Simple) {
        val error = new StatusMessage(
          reporter = None,
          message = s"Suggestion mode '$mode' not supported by simple mapping suggester!",
