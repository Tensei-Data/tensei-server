...
class AdvancedSemanticsSuggesterTest extends DefaultSpec with AdvancedSemanticsSuggester {
  ...
  describe("suggest") {
    ...
    describe("when given a different source and target dfasdl") {
      val dfasdlTestFiles = List("advanced.xml", "csv-with-head.xml")
      val dfasdlTargetFiles =
        HashMap("advanced.xml" -> "advanced-01.xml", "csv-with-head.xml" -> "csv-with-head.xml")
      val expectedCookbooks: Map[String, Cookbook] = Map(
        "advanced.xml" -> Cookbook(
          "advanced.xml",
          List(
            createDFASDLfromFile("source", s"/com/wegtam/tensei/server/suggesters/advanced.xml")
          ),
          Option(
            createDFASDLfromFile("target", s"/com/wegtam/tensei/server/suggesters/advanced-01.xml")
          ),
          List(
            Recipe(
              "auto-generated-1",
              MapOneToOne,
              List(
                MappingTransformation(List(ElementReference("source", "name6")),
                                      List(ElementReference("target", "vfname"))),
                MappingTransformation(List(ElementReference("source", "name7")),
                                      List(ElementReference("target", "vlname"))),
                MappingTransformation(List(ElementReference("source", "vendor_id")),
                                      List(ElementReference("target", "vendor_id")))
              )
            ),
            Recipe(
              "auto-generated-2",
              ...
              List(
                MappingTransformation(List(ElementReference("source", "name")),
                                      List(ElementReference("target", "title"))),
                MappingTransformation(List(ElementReference("source", "company_id")),
                                      List(ElementReference("target", "company_id"))),
                MappingTransformation(List(ElementReference("source", "date_entered")),
                                      List(ElementReference("target", "date_entered")))
              )
            ),
            Recipe(
              "auto-generated-3",
              MapOneToOne,
              List(
                MappingTransformation(List(ElementReference("source", "name5")),
                                      List(ElementReference("target", "lname"))),
                MappingTransformation(List(ElementReference("source", "name4")),
                                      List(ElementReference("target", "fname"))),
                MappingTransformation(List(ElementReference("source", "employee_id")),
                                      List(ElementReference("target", "employee_id")))
              )
            )
          )
        ),
          ...
          it("should return a cookbook with the correct recipes") {
            val cookbook = Cookbook(
              ...
              Option(
                createDFASDLfromFile(
                  "target",
                  s"/com/wegtam/tensei/server/suggesters/${dfasdlTargetFiles(dfasdlName)}"
                )
              ),
              List.empty[Recipe]
            )
